...
bool eval (string result_sha,Mail* mail) {
  ...
  system(("[ -d " + error_dir + " ] || mkdir " + error_dir).c_str());
  system(("[ -d " + plot_path_dir + " ] || mkdir " + plot_path_dir).c_str());
  system(("[ -d " + plot_error_dir + " ] || mkdir " + plot_error_dir).c_str());
  ...
  int seqNum = 5;
  for (int32_t i=seqNum; i<seqNum+1; i++) {
   
    // file name
    char file_name_gt[256], file_name_result[256];
    sprintf(file_name_gt,"%02d.txt",i);
	sprintf(file_name_result, "frame_trajectory_%02d_reformatted.txt",i);
    
    // read ground truth and result poses
    vector<Matrix> poses_gt     = loadPoses(file_name_gt);
    vector<Matrix> poses_result = loadPoses(file_name_result);
   
    // plot status
    mail->msg("Processing: %s, poses: %d/%d",file_name_gt,poses_result.size(),poses_gt.size());
	mail->msg("gt_size = %d, result_size = %d", poses_result.size(), poses_gt.size());
    
    // check for errors
    if (poses_gt.size()==0 || poses_result.size()!=poses_gt.size()) {
      mail->msg("ERROR: Couldn't read (all) poses of: %s", file_name_gt);
      return false;
    }
	
    // compute sequence errors    
    vector<errors> seq_err = calcSequenceErrors(poses_gt,poses_result);
    saveSequenceErrors(seq_err,error_dir + "/" + file_name_gt);
    ...
    if (i<=15) {
      ...
      savePathPlot(poses_gt,poses_result,plot_path_dir + "/" + file_name_gt);
