...
const GotoPose_FeedbackRefStruct = StructType({
...
const GotoPose_FeedbackRefArray = ArrayType(GotoPose_FeedbackRefStruct);
const GotoPose_FeedbackRefStructArray = StructType({
  data: GotoPose_FeedbackRefArray,
  size: ref.types.size_t,
  capacity: ref.types.size_t
});
// Define the wrapper class.
class GotoPose_FeedbackWrapper {
  constructor(other) {
    this._wrapperFields = {};
    if (typeof other === 'object' && other._refObject) {
      this._refObject = new GotoPose_FeedbackRefStruct(other._refObject.toObject());
  ...
  _initMembers() {
    this._refObject = new GotoPose_FeedbackRefStruct();
  ...
  static createFromRefObject(refObject) {
    let self = new GotoPose_FeedbackWrapper();
    self.copyRefObject(refObject);
    return self;
  }
  static createArray() {
    return new GotoPose_FeedbackArrayWrapper;
  }
  static get ArrayType() {
    return GotoPose_FeedbackArrayWrapper;
  }
  static get refObjectArrayType() {
    return GotoPose_FeedbackRefStructArray
  }
  static get refObjectType() {
    return GotoPose_FeedbackRefStruct;
  }
  toRawROS() {
    this.freeze(true);
  ...
  freeze(own = false, checkConsistency = false) {
    if (checkConsistency) {
      if (!this._statusIntialized) {
        throw new TypeError('Invalid argument: status in GotoPose_Feedback');
      }
      if (!this._messageIntialized) {
        throw new TypeError('Invalid argument: message in GotoPose_Feedback');
      }
      if (!this._distance_to_goalIntialized) {
        throw new TypeError('Invalid argument: distance_to_goal in GotoPose_Feedback');
      }
    }
    if (own) {
  ...
  static destoryRawROS(msg) {
    GotoPose_FeedbackWrapper.freeStruct(msg.refObject);
  }
  static type() {
    return {pkgName: 'trident_msgs', subFolder: 'action', interfaceName: 'GotoPose_Feedback'};
  }
  static isPrimitive() {
    return false;
  }
  ...
  copyRefObject(refObject) {
    this._refObject = new GotoPose_FeedbackRefStruct(refObject.toObject());
    this._statusIntialized = true;
    this._messageIntialized = true;
...
class GotoPose_FeedbackArrayWrapper {
  constructor(size = 0) {
    this._resize(size);
  }
  toRawROS() {
  ...
  fill(values) {
    ...
    values.forEach((value, index) => {
      if (value instanceof GotoPose_FeedbackWrapper) {
        ...
        this._wrappers[index] = new GotoPose_FeedbackWrapper(value);
      }
    });
  }
  ...
  set size(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to GotoPose_FeedbackArrayWrapper.size setter');
      return;
    }
  ...
  set capacity(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to GotoPose_FeedbackArrayWrapper.capacity setter');
    }
    return this._resize(value);
  }
  get refObject() {
  ...
  _resize(size) {
    ...
    this._refArray = new GotoPose_FeedbackRefArray(size);
    this._refObject = new GotoPose_FeedbackRefStructArray();
    this._refObject.size = size;
    this._refObject.capacity = size;
    this._wrappers = new Array();
    for (let i = 0; i < size; i++) {
      this._wrappers.push(new GotoPose_FeedbackWrapper());
    }
  }
  ...
  copy(other) {
    if (! (other instanceof GotoPose_FeedbackArrayWrapper)) {
      throw new TypeError('Invalid argument: should provide "GotoPose_FeedbackArrayWrapper".');
    }
  ...
  static freeArray(refObject) {
    ...
    for (let index = 0; index < refObject.size; index++) {
      GotoPose_FeedbackWrapper.freeStruct(refObjectArray[index]);
    }
  }
...
module.exports = GotoPose_FeedbackWrapper;
/*
