...
class MinimalServiceClient(Node):
    def __init__(self):
        ...
        self.sercli = self.create_client(Trigger, 'athena/motor_driver/motors/activate')
...
class TestTalkerListenerLink(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.node = rclpy.create_node('test_motors_activate')
        ...
        while rclpy.ok():
            ...
            if service_client.future.done():
                try:
                    ...
                    print(response)
                    self.assertTrue(response.success, "Motor driver activate success parameter should be: True")
                break
